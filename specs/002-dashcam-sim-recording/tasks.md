...
- [ ] T004 [P] PREREQUISITE (cross-repo): export video_codec `io` into the public umbrella in `/Users/moks/Develop/docker/ubuntu24/codes/video_codec/codec`: add `@video_codec//src/framework/io` to the deps of `video_codec` and `video_codec_hdrs` in `src/framework/public/BUILD.bazel`, relax visibility in `src/framework/io/BUILD.bazel` to allow the public target, copy `byte_sink.h` / `file_byte_sink.h` into `dist/host/include/video_codec/` (and `dist/android-arm64/include/video_codec/`), and add an umbrella compile smoke test under `tests/`; verify with `bazel build //src/framework/public:video_codec` + `bazel test //tests:all` in that repo (see `specs/002-dashcam-sim-recording/contracts/dependency-contract.md` D-1)
- [ ] T005 [P] Implement `media::record::Packet` in `src/framework/stream/packet.h`: variant payload (`video::codec::VideoFrame` RGBA / `video::codec::VideoPacket` / `SignalEvent`), `stream_name`, `pts_us`
- [ ] T006 [P] Implement `StreamBuffer` in `src/framework/stream/stream_buffer.{h,cc}`: bounded per-stream-name mailbox with single-consumer semantics (write/read, capacity limit, EOS flag)
- [ ] T007 Implement `PipelineRunner` in `src/framework/stream/pipeline_runner.{h,cc}`: instantiate nodes from `NodeRegistry::Create(type)`, connect `streams[]` to `StreamBuffer`s, drive a synchronous frame loop in topological order until `frame_count` (300 = 10s×30fps), then call `Close()` on every node in reverse topological order (depends on T005, T006)
...
- [ ] T008 [P] [US1] Unit test for software RGBA→I420 conversion in `src/tests/rgba_i420_test.cc` (known RGBA pattern → expected Y/U/V planes, BT.601, correct dimensions)
- [ ] T009 [P] [US1] Unit test for bitmap-font timestamp renderer in `src/tests/bitmap_font_test.cc` (glyphs blit at expected region for `YYYY-MM-DD HH:MM:SS`, two different times produce different pixels)
- [ ] T010 [P] [US1] Unit tests for source nodes in `src/tests/source_nodes_test.cc`: `StreamInputNode` decodes `dashcam_default.png` and emits one RGBA `VideoFrame` per Process with real-clock pts; `SignalSourceNode` emits periodic `kTick` events

### Implementation for User Story 1

- [ ] T011 [P] [US1] Implement software RGBA→I420 converter `src/nodes/video_encoder/rgba_i420.{h,cc}` (per-row RGB→YUV BT.601 + 2×2 chroma downsample; `VideoFrame` in `kRGBA`, out `kI420`) + register in `src/nodes/video_encoder/BUILD.bazel`
- [ ] T012 [P] [US1] Implement bitmap-font glyph table + renderer `src/nodes/ui_overlay/bitmap_font.{h,cc}` (embedded 5×7/8×13 digits + separators; draws white-on-semi-transparent-black into an RGBA buffer at a given rect) + register in `src/nodes/ui_overlay/BUILD.bazel`
- [ ] T013 [P] [US1] Implement `StreamInputNode` in `src/nodes/stream_input/stream_input_node.{h,cc}` (replace stub): `Open` decodes the default image via native_ui `Image::FromFile` + `CopyPixels` into RGBA, each `Process` emits a `Packet<VideoFrame>` (real-clock timestamp + `pts_us = frame_index × 1e6/30`) + register via `REGISTER_NODE` in `src/nodes/stream_input/BUILD.bazel`
- [ ] T014 [P] [US1] Implement `SignalSourceNode` in `src/nodes/signal_source/signal_source_node.{h,cc}` (replace stub): emits minimal `SignalEvent` (`kTick`) each Process on `output:signals` + register in `src/nodes/signal_source/BUILD.bazel`
- [ ] T015 [P] [US1] Implement `MultiViewLayoutNode` in `src/nodes/multi_view_layout/multi_view_layout_node.{h,cc}` (replace stub): build native_ui flex tree (`Container` + `ExternalImage`, fill frame), `Layout(w,h)`, software-blit the input frame pixels into the node's own RGBA `VideoFrame` buffer (frame fills whole frame; single-view only, multi-input reserved) + register in `src/nodes/multi_view_layout/BUILD.bazel` with `@native_ui` dep
- [ ] T016 [US1] Implement `UiOverlayNode` in `src/nodes/ui_overlay/ui_overlay_node.{h,cc}` (replace stub): compute timestamp position via native_ui flex (`Container` + `Text`, default bottom-right), render real-clock `%Y-%m-%d %H:%M:%S` text with `bitmap_font` into the incoming RGBA frame's top layer + register in `src/nodes/ui_overlay/BUILD.bazel` (depends on T012)
- [ ] T017 [US1] Implement `VideoEncoderNode` in `src/nodes/video_encoder/video_encoder_node.{h,cc}` (replace stub): software RGBA→I420 (`rgba_i420`) then `CodecFactory::CreateVideo` (H.264, `input_format: kI420`, 30fps) → `Init` → per-frame pull-mode `Encode(VideoFrame)` → `Packet<VideoPacket>` (Annex-B) → `Flush` at end + register in `src/nodes/video_encoder/BUILD.bazel` with `@video_codec//src/framework/public:video_codec` dep (depends on T011)
- [ ] T018 [P] [US1] Implement `RecorderNode` in `src/nodes/recorder/recorder_node.{h,cc}` (replace stub): single-session single-segment lifecycle, counts frames to `duration_seconds × fps` (300), forwards `VideoPacket`s to `output:clips`, finalizes on `Close` + register in `src/nodes/recorder/BUILD.bazel`
- [ ] T019 [US1] Implement `MuxerSinkNode` in `src/nodes/muxer_sink/muxer_sink_node.{h,cc}` (replace stub): `CodecFactory::CreateMuxer` (`MuxFormat::kMp4`, `fragmented=false` for file output) + `SetOutput(FileByteSink)` on temp file `out/.dashcam.mp4.tmp`, `Push(VideoPacket)` per packet, `Finish()` writes trailer, then atomic rename to `out/dashcam.mp4`; log when overwriting an existing file + register in `src/nodes/muxer_sink/BUILD.bazel` with `@video_codec` public umbrella dep (depends on T004)
- [ ] T020 [US1] Wire the runnable entry `src/examples/dashcam_record.cc`: load `dashcam_record.json` via `//src/framework/config:config`, run `PipelineRunner`, exit 0 on success (10s wall-clock paced at 30fps); add target to `src/examples/BUILD.bazel` with runfiles for the config + asset (depends on T007, T013–T019)
- [ ] T021 [US1] End-to-end test `src/tests/dashcam_record_test.cc`: drive `PipelineRunner` with a short frame count (e.g. 60 frames) via a test config, assert output MP4 exists with `ftyp`/`moov`/`mdat` boxes, expected frame count/duration, timestamp pixels change across frames, and overwrite of an existing file succeeds; register in `src/tests/BUILD.bazel` (depends on T020)
- [ ] T022 [US1] Verify existing dual-cam reference `src/examples/configs/recorder.json` still loads and runs through `src/examples/hello_graph.cc` with the real node registrations (single-view rendering; reference template stays config-valid, 8 nodes / 7 streams assertions unchanged)
...
- [ ] T023 [US3] Harden the entry-point error contract in `src/examples/dashcam_record.cc`: map node failures to exit code 1 with node-name + reason in stderr, config/argument errors to exit 2; success → 0
- [ ] T024 [P] [US3] Add input-missing / unsupported-format error path in `src/nodes/stream_input/stream_input_node.cc`: on decode failure return a locatable error including the image path (FR-008)
- [ ] T025 [P] [US3] Add output-unwritable detection + temp-file cleanup in `src/nodes/muxer_sink/muxer_sink_node.cc`: open failure or muxer `Finish` failure → report path, delete `out/.dashcam.mp4.tmp`, never leave a partial `out/dashcam.mp4` (FR-009)
- [ ] T026 [P] [US3] Add encode-failure propagation in `src/nodes/video_encoder/video_encoder_node.cc`: `CodecFactory::CreateVideo`/`Init`/`Encode`/`Flush` failures → named-node error mapping `kEncodeFailed` / `kPlatformUnsupported` / `kUnsupportedFormat`
- [ ] T027 [US3] Failure-path tests in `src/tests/dashcam_record_test.cc`: (a) missing input image → non-zero exit, stderr contains path, no output file; (b) unwritable output dir → non-zero exit, stderr contains path, no temp file left; (c) encode failure → non-zero exit, no partial `out/dashcam.mp4` (depends on T023–T026)
...
- [ ] T028 [P] Extend `mk/verify.mk` (and Makefile wiring) with a recording-artifact check step: build `//...`, run `bazel test //src/tests:all`, run `dashcam_record` and assert `out/dashcam.mp4` exists and is non-empty (SC-004)
- [ ] T029 [P] Update `media_record/doc/architecture/pipelines.md`: recorder topology reflects the real 7-node implementation, muxer writes via codec `Muxer` + `FileByteSink`, layout/OSD via native_ui flex layout + software draw
- [ ] T030 Run `quickstart.md` validation end-to-end from `media_record/media_record/`: `bazel build //...`, `bazel test //src/tests:all`, `make verify`; confirm default run exits 0 and `out/dashcam.mp4` is playable
...
- Foundational: T004 (cross-repo), T005, T006 run in parallel; T007 waits for T005+T006
...
Task: "T013 Implement StreamInputNode in src/nodes/stream_input/stream_input_node.{h,cc}"
Task: "T014 Implement SignalSourceNode in src/nodes/signal_source/signal_source_node.{h,cc}"
Task: "T015 Implement MultiViewLayoutNode in src/nodes/multi_view_layout/multi_view_layout_node.{h,cc}"
Task: "T018 Implement RecorderNode in src/nodes/recorder/recorder_node.{h,cc}"
```
...
2. Complete Phase 2: Foundational (T004–T007) — **CRITICAL**: T004 unblocks MuxerSinkNode
...
1. Complete Setup + Foundational → transport + runner + video_codec umbrella export ready
