...
- [x] T001 Create frame-transport module scaffold: `src/framework/transport/BUILD.bazel` declaring `cc_library(name = "transport")` (headers-only for now: `packet.h` / `stream_buffer.h` / `stream_node.h` / `pipeline_runner.h`), wire `@video_codec` core/types deps (module is `transport/` not `stream/` to avoid shadowing graph_runtime's `src/framework/stream/packet.h` include — see plan.md Structure Decision)
...
- [x] T005 [P] Implement `media::record::Packet` in `src/framework/transport/packet.h`: variant payload (`video::codec::VideoFrame` RGBA / `video::codec::VideoPacket` / `SignalEvent`), `stream_name`, `pts_us`
- [x] T006 [P] Implement `StreamBuffer` in `src/framework/transport/stream_buffer.h`: bounded per-stream-name mailbox with single-consumer semantics (write/read, capacity limit, EOS flag)
- [x] T007 Implement `PipelineRunner` in `src/framework/transport/pipeline_runner.{h,cc}` (+ `stream_node.h` data-flow node interface): instantiate nodes from `NodeRegistry::Create(type)`, connect `streams[]` to `StreamBuffer`s, drive a synchronous frame loop in topological order until `frame_count` (300 = 10s×30fps), EOS-mark + drain (nodes flush/finalize on EOS+empty), then call `Close()` on every node in reverse topological order; unit test `src/tests/pipeline_runner_test.cc` added (depends on T005, T006)
...
- [x] T010 [P] [US1] Unit tests for source nodes in `src/tests/source_nodes_test.cc`: `StreamInputNode` decodes `dashcam_default.png` and emits one RGBA `VideoFrame` per Process with real-clock pts; `SignalSourceNode` emits periodic `kTick` events
...
- [x] T014 [P] [US1] Implement `SignalSourceNode` in `src/nodes/signal_source/signal_source_node.{h,cc}` (replace stub): emits minimal `SignalEvent` (`kTick`) each Process on `output:signals` + register in `src/nodes/signal_source/BUILD.bazel`
...
- [x] T016 [US1] Implement `UiOverlayNode` in `src/nodes/ui_overlay/ui_overlay_node.{h,cc}` (replace stub): compute timestamp position via native_ui flex (`Container` + `Text`, default bottom-right), render real-clock `%Y-%m-%d %H:%M:%S` text with `bitmap_font` into the incoming RGBA frame's top layer + register in `src/nodes/ui_overlay/BUILD.bazel` (depends on T012)
- [x] T017 [US1] Implement `VideoEncoderNode` in `src/nodes/video_encoder/video_encoder_node.{h,cc}` (replace stub): software RGBA→I420 (`rgba_i420`) then `CodecFactory::CreateVideo` (H.264, `input_format: kI420`, 30fps) → `Init` → per-frame pull-mode `Encode(VideoFrame)` → `Packet<VideoPacket>` (Annex-B) → `Flush` at end + register in `src/nodes/video_encoder/BUILD.bazel` with `@video_codec//src/framework/public:video_codec` dep (depends on T011)
- [x] T018 [P] [US1] Implement `RecorderNode` in `src/nodes/recorder/recorder_node.{h,cc}` (replace stub): single-session single-segment lifecycle, counts frames to `duration_seconds × fps` (300), forwards `VideoPacket`s to `output:clips`, finalizes on `Close` + register in `src/nodes/recorder/BUILD.bazel`
...
- [x] T020 [US1] Wire the runnable entry `src/examples/dashcam_record.cc`: load `dashcam_record.json` via `//src/framework/config:config`, run `PipelineRunner`, exit 0 on success (10s wall-clock paced at 30fps); add target to `src/examples/BUILD.bazel` with runfiles for the config + asset (depends on T007, T013–T019)
...
- [x] T022 [US1] Verify existing dual-cam reference `src/examples/configs/recorder.json` still loads and runs through `src/examples/hello_graph.cc` with the real node registrations (single-view rendering; reference template stays config-valid, 8 nodes / 7 streams assertions unchanged)
...
- [x] T023 [US3] Harden the entry-point error contract in `src/examples/dashcam_record.cc`: map node failures to exit code 1 with node-name + reason in stderr, config/argument errors to exit 2; success → 0
- [x] T024 [P] [US3] Add input-missing / unsupported-format error path in `src/nodes/stream_input/stream_input_node.cc`: on decode failure return a locatable error including the image path (FR-008)
...
- [x] T029 [P] Update `media_record/doc/architecture/pipelines.md`: recorder topology reflects the real 7-node implementation, muxer writes via codec `Muxer` + `FileByteSink`, layout/OSD via native_ui flex layout + software draw
