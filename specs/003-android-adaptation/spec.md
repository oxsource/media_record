...
- Q: native_ui `ExternalImage` 当前硬编码 kCPU（无 GPU 零拷贝路径），如何支持 Android surface 模式？ → A: 扩展 `ExternalImage` 支持 GPU（`RenderBackend::kGPU` + `RenderContext`），走 `Image::FromBuffer` 的 `AHwb::ToGpuImage` 零拷贝导入。
...
在 Android 上，Dashcam 背景图不再通过 CPU 解码 → RGBA 拷贝路径，而是：解码为 RGBA → 写入 `AHardwareBuffer`（`AHwb::AllocateRGBA` + `WriteRGBA`）→ 用 `ExternalImage` widget 加载显示。surface 模式下经 `RenderBackend::kGPU` 走 `AHwb::ToGpuImage` 零拷贝纹理导入，避免 CPU 回读。
...
1. **Given** 背景图解码成功，**When** 写入 AHardwareBuffer 并由 ExternalImage 加载，**Then** 画面显示完整背景图，尺寸正确
2. **Given** surface 模式（GPU），**When** 渲染一帧，**Then** 背景图经 `ToGpuImage` 零拷贝导入，无逐帧 CPU 拷贝
...
- **FR-002**: `ExternalImage` MUST 支持 GPU 渲染后端（`RenderBackend::kGPU` + `RenderContext`），经 `AHwb::ToGpuImage` 零拷贝导入；host/CPU 默认路径保持不变。
...
- **ExternalImage**: native_ui widget，加载并显示 AHWB 图像，新增 GPU 后端。
...
- **SC-001**: Android 上背景图经 AHWB + ExternalImage（GPU）显示，无逐帧 CPU 拷贝。
- **SC-002**: Android 上端到端录制产出可播放 H.264 MP4，画面含背景 + 跳动小狗 + 时间戳。
- **SC-003**: host 上 `make verify` 全部通过，行为与适配前一致。
- **SC-004**: `ExternalImage` 支持 GPU 与 CPU 双后端，且 CPU 默认路径无回归。
- **SC-005**: 任一失败场景（AHWB / MediaCodec / RenderContext 创建失败）返回可定位错误，安全退出。

## Assumptions

- 三仓库已具备 Android 底层能力：native_ui 已有 `AHwb`/`RenderContext`/`Surface::Create(ctx)`，video_codec 已有 MediaCodec backend 与 `CreateInputSurface()`/`Poll()`/`input_surface` 契约。本 feature 聚焦 media_record 节点接线 + native_ui `ExternalImage` GPU 扩展。
...
- Android 上 `ExternalImage` 的 GPU 零拷贝为本次目标；若个别设备不支持 GPU 导入，可回退 CPU 路径（功能正确优先，性能优化次之）。
